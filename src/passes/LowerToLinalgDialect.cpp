...
/// Create a linalg.generic operation with the given indexing maps and iterator
/// types.
linalg::GenericOp createGenericOp(
    OpBuilder &builder, Location loc, ArrayRef<Value> inputs,
    ArrayRef<Value> outputs, ArrayRef<AffineMap> indexingMaps,
    ArrayRef<utils::IteratorType> iteratorTypes,
    function_ref<void(OpBuilder &, Location, ValueRange)> bodyBuilder) {
  return builder.create<linalg::GenericOp>(loc, outputs[0].getType(), inputs,
...
struct DenseToLinalgLowering : public OpRewritePattern<DenseOp> {
  ...
  LogicalResult matchAndRewrite(DenseOp op,
    ...
    Value filledOutput =
        rewriter.create<linalg::FillOp>(loc, zero, output).getResult(0);
    ...
    if (bias) {
      ...
      AffineMap biasMap = AffineMap::get(2, 0, {rewriter.getAffineDimExpr(1)},
                                         rewriter.getContext());
      AffineMap outputMap =
          AffineMap::getMultiDimIdentityMap(2, rewriter.getContext());
      ...
      auto biasOp = createGenericOp(
          rewriter, loc, {matmulResult, bias}, {filledOutput}, indexingMaps,
...
struct TanhOpLinalgLowering : public OpRewritePattern<TanhOp> {
  ...
  LogicalResult matchAndRewrite(TanhOp op,
    ...
    math::TanhOp tanhOp = rewriter.create<math::TanhOp>(loc, resultType, input);
...
struct SoftmaxOpLinalgLowering : public OpRewritePattern<SoftmaxOp> {
  ...
  LogicalResult matchAndRewrite(SoftmaxOp op,
    ...
    if (sumShape.empty())
      sumShape.push_back(1); // Handle 1D case

    Value sumInit = rewriter.create<tensor::EmptyOp>(
        loc, sumShape, resultType.getElementType());
    Value zero = rewriter.create<arith::ConstantOp>(
        loc, rewriter.getZeroAttr(resultType.getElementType()));
    Value sumFilled =
        rewriter.create<linalg::FillOp>(loc, zero, sumInit).getResult(0);

    // Create reduction along last dimension
    SmallVector<int64_t> reductionDims = {
        static_cast<int64_t>(resultType.getRank() - 1)};
    ...
    AffineMap inputMap = AffineMap::getMultiDimIdentityMap(
        resultType.getRank(), rewriter.getContext());
    AffineMap sumMap =
        AffineMap::get(resultType.getRank(), 0, {rewriter.getAffineDimExpr(0)},
                       rewriter.getContext()); // Broadcast sum
    ...
    auto divOp =
        createGenericOp(rewriter, loc, {expOp.getResult(0), sumOp.getResult(0)},
                        {output}, divIndexingMaps, divIteratorTypes,
                        [](OpBuilder &b, Location loc, ValueRange args) {
                          Value div =
  ...
  LogicalResult matchAndRewrite(TransposeOp op,
    ...
    if (inputType.getRank() != 2) {
      return rewriter.notifyMatchFailure(
          op, "transpose only supports 2D tensors currently");
    }
...
/// Lower nn.add to linalg.generic (element-wise addition with broadcasting
/// support)
struct AddOpLinalgLowering : public OpRewritePattern<AddOp> {
  ...
  LogicalResult matchAndRewrite(AddOp op,
    ...
    // TODO : Handle this more generically, or have a sparate pass for
    // broadcasting. Handle broadcasting case: [1x5] + [5] -> [1x5]
    if (lhsType.getRank() == 2 && rhsType.getRank() == 1) {
      ...
      AffineMap rhsMap =
          AffineMap::get(2, 0, {rewriter.getAffineDimExpr(1)}, context);
      ...
      AffineMap identityMap =
          AffineMap::getMultiDimIdentityMap(resultType.getRank(), context);
      SmallVector<AffineMap> indexingMaps = {identityMap, identityMap,
                                             identityMap};
...
struct FuncOpLowering : public OpRewritePattern<FuncOp> {
  ...
  LogicalResult matchAndRewrite(FuncOp op,
    ...
    auto newFuncOp =
        rewriter.create<func::FuncOp>(op.getLoc(), op.getSymName(), funcType);
...
struct LowerToLinalgPass
  ...
  void runOnOperation() override {
    ...
    patterns.add<DenseToLinalgLowering, ReluOpLinalgLowering,
                 SigmoidOpLinalgLowering, TanhOpLinalgLowering,
                 SoftmaxOpLinalgLowering, TransposeOpLinalgLowering,
                 MatmulOpLinalgLowering, AddOpLinalgLowering, FuncOpLowering,
