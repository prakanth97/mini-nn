...
void NNDialect::printType(mlir::Type type,
                          mlir::DialectAsmPrinter &printer) const {
  // For now, do nothing as we don't have custom types
}

mlir::Attribute NNDialect::parseAttribute(mlir::DialectAsmParser &parser,
                                          mlir::Type type) const {
  // For now, just return nullptr as we don't have custom attributes
  return nullptr;
}

void NNDialect::printAttribute(mlir::Attribute attr,
                               mlir::DialectAsmPrinter &printer) const {
...
void DenseOp::inferShapes() {
  ...
  outputShape.append(inputShape.begin(),
                     inputShape.end() - 1); // All dims except last
...
void Conv1DOp::inferShapes() {
  ...
  SmallVector<int64_t> outputShape = {inputShape[0], outputLength,
                                      kernelShape[2]};
...
void Conv2DOp::inferShapes() {
  ...
  // Weight shape: [kernel_height, kernel_width, input_channels,
  // output_channels] Output shape: [batch_size, output_height, output_width,
  // output_channels]
  ...
  // Create the output shape: [batch_size, output_height, output_width,
  // output_channels]
  SmallVector<int64_t> outputShape = {inputShape[0], outputHeight, outputWidth,
                                      kernelShape[3]};
...
void TransposeOp::inferShapes() {
  ...
  std::swap(outputShape[outputShape.size() - 1],
...
void MatmulOp::inferShapes() {
  ...
  if (lhsShape.size() < 2 || rhsShape.size() < 2 ||
      lhsShape[lhsShape.size() - 1] != rhsShape[rhsShape.size() - 2]) {
    return; // Invalid shapes, let the verifier handle this
  }
  ...
  if (frontElements.empty()) {
    outputShape = {lhsShape[lhsShape.size() - 2],
                   rhsShape[rhsShape.size() - 1]};
  } else {
...
void SigmoidOp::inferShapes() { getResult().setType(getInput().getType()); }

//===----------------------------------------------------------------------===//
// TanhOp
//===----------------------------------------------------------------------===//

void TanhOp::inferShapes() { getResult().setType(getInput().getType()); }

//===----------------------------------------------------------------------===//
// SoftmaxOp
//===----------------------------------------------------------------------===//

void SoftmaxOp::inferShapes() { getResult().setType(getInput().getType()); }
