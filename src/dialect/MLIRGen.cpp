...
private:
  ...
  SmallVector<mlir::Type> getLayerParameterTypes(LayerDefinition *layerDef) {
    ...
    if (layerDef->layerType == "dense") {
      ...
      if (layerDef->parameters.size() >= 2) {
        auto *inputDim = dynamic_cast<NumberLiteral*>(layerDef->parameters[0].get());
        auto *outputDim = dynamic_cast<NumberLiteral*>(layerDef->parameters[1].get());
        
        if (inputDim && outputDim) {
          // Weight matrix: [input_dim, output_dim]
          SmallVector<int64_t> weightShape = {std::stoll(inputDim->value), std::stoll(outputDim->value)};
          paramTypes.push_back(mlir::RankedTensorType::get(weightShape, builder.getF32Type()));
          
          // Bias vector: [output_dim]
          SmallVector<int64_t> biasShape = {std::stoll(outputDim->value)};
          paramTypes.push_back(mlir::RankedTensorType::get(biasShape, builder.getF32Type()));
        }
      }
    }
  ...
  mlir::nn::FuncOp mlirGen(FunctionDefinition &funcDef) {
    ...
    // Add layer parameters (weights and biases) for each layer used in the function
    SmallVector<std::string> layerParamNames;
    for (auto &stmt : funcDef.body) {
      if (auto *assign = dynamic_cast<Assignment*>(stmt.get())) {
        if (auto *funcCall = dynamic_cast<FunctionCall*>(assign->value.get())) {
          ...
          if (layerMap.find(funcCall->functionName) != layerMap.end()) {
            ...
            for (size_t i = 0; i < layerTypes.size(); ++i) {
              ...
              argLocs.push_back(builder.getUnknownLoc());
              layerParamNames.push_back(funcCall->functionName + "_param" + std::to_string(i));
            }
          }
        }
      }
    ...
    auto funcType = mlir::FunctionType::get(&context, argTypes, getType(funcDef.returnType.get()));
    ...
    auto function = builder.create<mlir::nn::FuncOp>(loc, funcName, funcTypeAttr);
  ...
  mlir::Value mlirGen(FunctionCall &call) {
    ...
    if (layerMap.find(call.functionName) != layerMap.end()) {
      ...
      if (layerDef->layerType == "dense") {
        ...
        auto denseOp = builder.create<mlir::nn::DenseOp>(loc, input.getType(), input, weight, bias);
    ...
    if (call.isBuiltin) {
      ...
      if (call.functionName == "relu") {
        auto reluOp = builder.create<mlir::nn::ReluOp>(loc, input.getType(), input);
        return reluOp.getResult();
      } else if (call.functionName == "sigmoid") {
        auto sigmoidOp = builder.create<mlir::nn::SigmoidOp>(loc, input.getType(), input);
        return sigmoidOp.getResult();
      } else if (call.functionName == "tanh") {
        auto tanhOp = builder.create<mlir::nn::TanhOp>(loc, input.getType(), input);
  ...
  mlir::Value mlirGen(NumberLiteral &num) {
    if (num.isFloat) {
      ...
      return builder.create<mlir::arith::ConstantOp>(builder.getUnknownLoc(), value);
    } else {
