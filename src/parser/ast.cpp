...
std::string LayerDefinition::toString(int indent) const {
  ...
  result += getIndent(indent + 1) +
            "hasKeyword: " + (hasLayerKeyword ? "true" : "false") + "\n";
...
std::string FunctionDefinition::toString(int indent) const {
  ...
  for (const auto &param : parameters) {
    if (param.first) {
      result += getIndent(indent + 2) + "(" + param.first->toString(0) +
                ", \"" + param.second + "\")\n";
    } else {
  ...
  result += getIndent(indent + 1) + "]\n";
  result +=
      getIndent(indent + 1) + "returnType: " + returnType->toString(0) + "\n";
...
std::string Assignment::toString(int indent) const {
  ...
  result +=
      getIndent(indent + 1) + "value: " + value->toString(indent + 1) + "\n";
  result += getIndent(indent) + "}";
  return result;
}
...
std::string NumberLiteral::toString(int indent) const {
  return getIndent(indent) + "NumberLiteral: " + value + " (" +
         (isFloat ? "float" : "int") + ")";
}
