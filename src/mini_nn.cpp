...
int main(int argc, char** argv) {
  ...
  if (last_slash != std::string::npos) {
    location_prefix = input_file_path.substr(0, last_slash + 1);  // Include the trailing slash
  }
  ...
  std::cout << "Output directory: " << (location_prefix.empty() ? "./" : location_prefix) << std::endl;
  ...
  // Load specific dialects we need, without loadAllAvailableDialects() which might cause issues
  ...
  if (EC) {
    std::cerr << "Failed to open file for writing: " << linalg_file << std::endl;
    return 1;
  }
  ...
  std::string mlir_opt_path = "/Users/u5624836/Desktop/PhD/repos/llvm-project/build/bin/mlir-opt";
  
  // Run first set of passes (bufferization and optimization)
  std::string bufferized_file = location_prefix + base_filename + "_bufferized.mlir";
  std::string mlir_passes_cmd = mlir_opt_path + " " + linalg_file + 
  ...
  // Run second set of passes (lowering to LLVM)
  std::string llvm_file = location_prefix + base_filename + "_llvm.mlir";
  std::string lower_passes_cmd = mlir_opt_path + " " + bufferized_file +
